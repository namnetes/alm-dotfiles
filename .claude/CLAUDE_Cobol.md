# Claude Instructions — IBM Mainframe (z/OS)

Standards for IBM Enterprise COBOL 6.5, JCL z/OS JES2, and z/OS subsystem
integration (IBM Db2, IBM CICS TS, IBM IMS, VSAM for z/OS, DFSORT).
Loaded automatically when working in this directory or any subdirectory.

---

## Mainframe — IBM z/OS Product Naming
...
## Mainframe — IBM Enterprise COBOL
...
> _"This format is most efficient when you code an **odd** number of digits
> in the PICTURE description, so that the leftmost byte is fully used."_
...
- Order `WHEN` clauses from the **most frequent** to the least frequent:
  the compiler generates a branch table and runtime skips less-common paths.
...
| Memory      | Always resident         | Loaded on demand         |
...
- **Always** use `CALL variable` (dynamic) to call user-written programs.
  The program name is stored in a `PIC X(8)` WORKING-STORAGE field and
  resolved at runtime. This decouples the caller from the link-edit and
  allows replacing or patching the called module without relinking.
- **Only** use `CALL 'literal'` (static) for IBM-supplied modules whose
  names are fixed and will never change:
  `'CBLTDLI'`, `'XMLINIT'`, `'CEE3DMP'`, `'CEE3ABD'`, etc.
- Never code a user program name as a literal in a `CALL` statement.
- `CANCEL variable` frees the module from memory and forces
  re-initialisation on the next load; use sparingly (reload cost).
...
- Organise in named SECTIONS; each section has a single logical
  responsibility (one cohesive function per section).
...
- Every batch program terminates with `STOP RUN`. Every subprogram terminates
  with `GOBACK`. Never use `STOP RUN` in CICS programs (use
...
Code `ON OVERFLOW` on `STRING` and `UNSTRING` to guard against buffer
overflow.
...
- Always declare `INDEXED BY` on tables used with SEARCH / SEARCH ALL:
  index-based access is faster than subscript-based (no address
  multiplication).
...
files.** For sequential processing, end-of-file and empty-file detection
are handled directly by the `AT END` phrase of the `READ` statement and a
record counter — no FILE STATUS field is needed or desirable:

- `AT END` fires immediately on the first `READ` when the file is empty;
  checking the counter after the read loop reveals whether any records
  were processed.
- Relying on FILE STATUS for EOF (`'10'`) alongside `AT END` is redundant
  and introduces two parallel control paths that must be kept in sync.

**Other rules:**

- Always code `BLOCK CONTAINS 0 RECORDS` in every FD for sequential files:
  DFSMS chooses the optimal block size automatically.
- Use `READ … INTO` and `WRITE … FROM` to keep the FD record area as a pure
  I/O buffer; do business logic on WORKING-STORAGE copies.
- Verify that JCL attributes (RECFM, LRECL) match the FD declaration; a
  mismatch causes an abend at OPEN.
- For variable-length records (FORMAT V): the JCL LRECL must be 4 bytes
  greater than the maximum COBOL record length.
...
**Error-handling hierarchy for sequential files** (first matching
mechanism wins):

1. `AT END` / `NOT AT END` phrase on `READ` — canonical EOF detection
2. `EXCEPTION/ERROR` declarative procedure — all other I/O errors
3. If none: runtime terminates with message
...
- Load a KSDS with `OPEN OUTPUT` + `ACCESS IS SEQUENTIAL` (never with RANDOM
  — disables free-space management, future inserts will be slower).
- Sequential access > dynamic > random: random access reads the index for
  every request.
- Code `BUFND` (data buffers) and `BUFNI` (index buffers) in the JCL AMP
  parameter when performance is critical.
- Minimise alternate indexes: every WRITE/REWRITE/DELETE updates all of them.
- Periodically reorganise (EXPORT/IMPORT or REPRO) to eliminate CA splits.
- Sequential DELETE: always `READ` the record first, then `DELETE` (otherwise
  FILE STATUS `92`).
- In multithreading: always open and close a VSAM file from the **same
  thread** (opening and closing from different threads → status 90).
...
Whenever the sort can be expressed as a standalone step, invoke DFSORT
directly in JCL: cleaner separation of concerns, better performance, and
no COBOL SORT infrastructure to maintain.
...
Use the COBOL SORT verb **only** when the sort logic is inseparable from the
program (e.g. FORMAT 2 in-memory table sort, or a complex INPUT/OUTPUT
PROCEDURE that cannot be externalized).

#### Rules when COBOL SORT is unavoidable

- Always check `SORT-RETURN` after every `SORT`/`MERGE` statement:
  `0` = success, `16` = failure.
- Always code `FASTSRT` compiler option: delegates I/O to DFSORT/ICETOOL
  for better performance.
- Declare SORTWK DD statements in JCL for every SORT step (minimum 3).
- SORT restrictions:
  - **Absolute ban** in programs compiled with `THREAD`.
  - **Absolute ban** under CICS for FORMAT 1 SORT (with USING/GIVING).
    Under CICS: use FORMAT 2 SORT (in-memory table sort) or an
    external sort utility invoked via CICS START.
- Never take program-level checkpoints during an active SORT/MERGE.
- Inside SORT INPUT/OUTPUT procedures:
  - No nested `SORT` or `MERGE`.
  - No transfer of control out of the procedure.
  - No Language Environment condition handlers.
...
variables; compile with `RENT`; never use `STOP RUN` (use `EXEC CICS
RETURN END-EXEC`); always handle abends explicitly.
...
`JSON GENERATE` statements for production code. These statements are
convenient for prototyping but are limited in functionality (no schema
validation, no XPath/JSONPath, static structure only, poor diagnostics on
large or malformed documents). Always prefer a dedicated z/OS parser.

**XML — preferred parsers**

| Parser                       | Product / availability               | When to use                                                                                                       |
| ---------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| **IBM XML Toolkit for z/OS** | 5655-J51 — also free SMP/E download  | Full-featured: SAX2, DOM, XPath 1.0, XSLT 1.0, XML Schema (XSD) validation. Use for any complex or validated XML. |
| **z/OS XML System Services** | Part of z/OS base (no extra product) | Lightweight streaming (SAX-like) parser. Use for high-throughput batch XML when validation is not required.       |

The IBM XML Toolkit for z/OS wraps **Apache Xerces-C**. COBOL programs
invoke it via `CALL` to C/C++ entry points compiled into load modules.
Always link the toolkit load library in `STEPLIB`/`JOBLIB` or the LNKLST.

```cobol
*> ── Skeleton: invoke IBM XML Toolkit SAX parser from COBOL ─────────
       WORKING-STORAGE SECTION.
       01  WS-XML-PARSER-HANDLE   PIC S9(9) COMP-5.
       01  WS-XML-RC              PIC S9(9) COMP-5.

       PROCEDURE DIVISION.
*>     Initialise parser handle (Xerces-C wrapper entry point)
           CALL 'XMLINIT' USING WS-XML-PARSER-HANDLE
                          RETURNING WS-XML-RC.
           IF WS-XML-RC NOT = 0
               PERFORM 9900-XML-INIT-ERROR
           END-IF.
```
...
| -------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **IBM z/OS Connect**                   | REST API integration     | Strategic IBM solution. Generates JSON binding automatically from COBOL copybooks. Use for all external REST/JSON APIs.                                                            |
| **CICS DFHJSON service**               | IBM CICS TS for z/OS 6.2 | Within CICS: use `EXEC CICS PUT CONTAINER` / `GET CONTAINER` with JSON channels. DFHJSON maps COBOL structures to JSON schemas.                                                    |
| **IBM Db2 for z/OS 13 JSON functions** | DB2 context              | Use `JSON_TABLE`, `JSON_VALUE`, `JSON_QUERY`, `JSON_EXISTS` in SQL instead of parsing JSON in COBOL.                                                                               |
| **YAJL for z/OS**                      | Batch / UNIX SS          | Open-source (Yet Another JSON Library) ported to z/OS. Streaming SAX-like API, callable from COBOL via `CALL`. Use for batch programs that must parse dynamic or schema-less JSON. |

```cobol
*> ── IBM Db2 for z/OS 13: parse JSON in SQL, no COBOL parsing needed ──
           EXEC SQL
...
| ------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `XML PARSE`                    | Unit tests, one-off utilities, structure trivial and fully known at compile time, no validation required |
| `JSON PARSE` / `JSON GENERATE` | Simple fixed-structure messages, internal COBOL-to-COBOL data exchange, prototyping                      |
...
Always specify compiler options in the source program `PROCESS` statement
or in the JCL `PARM` of the `IGYWCL` step.
...
| `RMODE(ANY)`     | Residence above 16 MB line                           | Default; use RMODE(24) only if required |
| `AMODE(31)`      | 31-bit addressing                                    | Standard; use AMODE(64) for 64-bit      |
...
| `ARCH(10)`       | Generate z/Architecture instructions                 | Production                              |
| `TUNE(10)`       | Tune for target processor                            | Production                              |
...
| `TRAP(ON)`       | Intercept runtime conditions (overflow, bad address) | Always                                  |
| `SQL`            | Activate DB2 precompiler pass                        | IBM Db2 programs                        |
...
- Use `REPLACING` to prefix data names and avoid collisions when the
  same copybook is included multiple times.
...
| Raw literals (`'A'`, `12`, `'00'`) in PROCEDURE DIVISION | Replace with 88-level condition names or named `77`/`01` constants                                                    |
| `FILE STATUS IS` declared on a sequential file SELECT    | Remove it; use `AT END`/`NOT AT END` on READ for EOF; use a counter for empty-file detection                          |
| FILE STATUS never checked (VSAM / indexed files)         | Add double FILE STATUS and check with 88-level names after every I/O verb                                             |
| Cryptic names (X, Y, AA, BB)                             | Rename with descriptive hyphenated names + section prefix                                                             |
| WORKING-STORAGE in a CICS/RENT program for work areas    | Move to LOCAL-STORAGE                                                                                                 |
...
| COBOL SORT verb in a new program                         | Replace with a DFSORT step in JCL; keep COBOL SORT only if the INPUT/OUTPUT PROCEDURE is inseparable from the program |
| SORT without `SORT-RETURN` check                         | Add check immediately after SORT/MERGE                                                                                |
| `CALL 'literal'` to a user program                       | Replace with `CALL variable` (dynamic); store the name in a `PIC X(8)` WORKING-STORAGE field                          |
...
Use `FUNCTION` keyword; no `USING` clause required. Functions can appear
anywhere an identifier or literal is valid.

**Date and time**

| Function                                              | Returns             | Notes                                               |
| ----------------------------------------------------- | ------------------- | --------------------------------------------------- |
| `FUNCTION CURRENT-DATE`                               | `PIC X(21)`         | `YYYYMMDDHHMMSSCChh mm` — last 5 chars = UTC offset |
| `FUNCTION WHEN-COMPILED`                              | `PIC X(21)`         | Same format, fixed at compile time                  |
...
| `FUNCTION SECONDS-PAST-MIDNIGHT`                      | Integer             | Seconds since midnight                              |
...
```cobol
* Extract YYYYMMDD from CURRENT-DATE
       05  WS-CURRENT-DATE.
           10  WS-DATE        PIC 9(8).   *> YYYYMMDD
           10  WS-TIME        PIC 9(6).   *> HHMMSS
           10  WS-HUNDREDTHS  PIC 9(2).   *> CC
           10  WS-UTC-SIGN    PIC X.
           10  WS-UTC-OFFSET  PIC 9(4).
           MOVE FUNCTION CURRENT-DATE TO WS-CURRENT-DATE.

* Compute difference in days between two dates
       COMPUTE WS-DAYS-DIFF =
           FUNCTION INTEGER-OF-DATE(WS-DATE-END)
         - FUNCTION INTEGER-OF-DATE(WS-DATE-START).
```
...
| `FUNCTION TEST-DAY-YYYYDDD(n)`     | Integer | 0 = valid, >0 = error position  |
...
| `FUNCTION TRIM(str [LEADING\|TRAILING])`  | Variable     | Removes spaces; default BOTH            |
...
| `FUNCTION CONCATENATE(a, b, …)`           | String       | IBM extension — joins strings           |
...
| `FUNCTION NUMVAL-C(str, sym)` | Numeric | Parses `'$1,234.56'` — sym=`'$'` |
| `FUNCTION NUMVAL-F(str)`      | Float   | Parses `'1.23E+04'`              |

---

## Mainframe — JCL (z/OS JES2)
